...
const ProductProvider = ({ children }) => {
  ...
  const getData = async () => {
    setLoading(true);
    try {
      // axios'tan gelenin içinde her zaman kelime olarak "data" olduğu için bu şekilde const { data } direkt datayı çektik içinden
...
// [search] sayesinde search barda onChange ile her harf girildikçe useEffecti tekrar çalıştırıyoruz 
  return <ProductContext.Provider value={{products,loading,search,setSearch}}>{ children }</ProductContext.Provider>;
  // hangi verilerin erişilebilir olmasını istiyorsak onları value={{ ile dışarı açıyoruz. dikkat edersen getData fonksyonunu dışarı açmadık
  // ama search,setSearch dışarı açtık. useEffect'te  [search]); olduğundan diğer dosyalarda search değişkenini değiştirerek bu dosyadaki  getData fonksyonunu tetikleyebiliyoruz
};
