...
const ProductDetail = () => {
  ...
  const { state, search } = useLocation();//! (bak ProductCard.jsx) navigate ile taşınan veriyi useLocation() hooku ile karşılyabiliyoruz. urlde yer alan parametreleri search ile yakalayabiliyoruz.
  // search url'de ?detail=${item.id} kısmını verir. bak ProductCard.jsx
  const navigate = useNavigate();
  console.log(search);
  console.log(state);
  // const getDetailData = async () => {
    // {search.split("=")[1]} amacı: search bunu dönderir ?detail=${item.id} örneğin bu ?detail=5 olsun. 
    //  apı isteği için https://dummyjson.com/products/5 yapmamız lazım.
    // ?detail=5 bundan 5'i {search.split("=")[1]} bu şekilde alıyoruz. eşittirden böl 2 elemanlı dizi olsun. [1] 2. elemanı yani 5'i al.
  ...
  //  <ErrorPage />'e gitmek için browserdan url sonu saçma bişeyler ekleyebilirsin
        ...
        <article className="mx-auto w-full block lg:flex mt-4 h-full 2xl:h-[70vh]  shadow-lg border rounded-md duration-300 hover:shadow-sm">
          <div class="grid grid-rows-4 gap-2 h-full w-full lg:w-7/12 p-4">
            ...
            <div className="grid grid-cols-3 gap-4 row-span-1">
              {/* images.slice(0, images.length - 2) en fazla 3 resim göstermesi için çünkü apı kaynağında images dizi eleman sayısı değişkenlik olmakla birlikte 5'e kadar çıkabiliyor */}
